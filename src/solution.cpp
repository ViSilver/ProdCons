...
void * prodCostFunc ( void *  id ) // To be implemented a prodFunc for each type of producer
 {
    int i;
...
void * prodCrimeFunc ( void *  id ) // To be implemented a prodFunc for each type of producer
{
    int i;
...
void MapAnalyzer ( int threads, const TCostProblem * (* costFunc) ( void ), const TCrimeProblem* (* crimeFunc) ( void ) )
 {
    //    
     int prod, cons = threads, i;
     pthread_t      * thrID;
     pthread_attr_t   attr;
     
     // Creating the data for the producers
     ProdCrimeStruct crimeStruct;
     ProdCostStruct costStruct;
     
     crimeStruct.tCrimeFunc = crimeFunc;
     costStruct.tCostFunc = costFunc;
    ...
    // Initialization of global variables
     ...
     thrID = (pthread_t *) malloc ( sizeof ( *thrID ) * ( prod + threads ) );
     ...
     if ( pthread_create (&thrID[0], &attr, prodCostFunc, (void*) (&costStruct)) )
     {
         perror ( "pthread_create" );
         return;
     }
     if ( pthread_create (&thrID[1], &attr, prodCrimeFunc, (void*) (&crimeStruct)) )
     {
         perror ( "pthread_create" );
         return;
     }
